...
- **Bracket** — a radial knockout tree (SVG): flag-badge nodes (real vector flag art filling the node circle exactly, not an emoji glyph — iOS Safari renders flag emoji as a blank box inside SVG `<text>`) connected by standard bracket elbow lines (arc + radial line, like a normal left-to-right bracket bent into a circle), not a filled pie/donut chart. Rings run Round of 32 on the outside in to Semi-final, with the Final as the centre bullseye (shows the champion's flag + name once it's decided). A node rings green once that team has won its tie, red once it's lost; the winner's branch of the connector into the next round highlights green the same way, while the loser's branch highlights red only up to where the two paths met — it never continues inward, since that branch stops there. Team name and coach (if drafted in this pool) are two separate lines stacked in the label's own rotated frame, so they can never collide with each other or the flag and are never upside down at any position around the wheel. Still-undecided placeholder slots get a bare dashed node with no text at all, rather than repeating "Round of 32 8 Winner" across a whole ring; the centre stays a plain "FINAL" until both finalists are actually known, rather than labelling two empty slots "Match 1"/"Match 2". Every ring's title shows once in a reserved gap, not repeated on every node. Tap a node for a detail card below with score and live/FT status — nothing shows by default. 3rd place is a small tappable note beneath the wheel since it isn't part of the winners' line. Ring/node order is reconstructed from the *real* bracket lineage (matching resolved teams back to their source fixture, and parsing ESPN's placeholder names for unresolved ones) — not just chronological kickoff order, which doesn't always match the actual pairing.
