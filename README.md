...
- **Bracket** — a radial knockout tree (SVG): small flag-circle nodes connected by standard bracket elbow lines (arc + radial line, like a normal left-to-right bracket bent into a circle), not a filled pie/donut chart. Rings run Round of 32 on the outside in to Semi-final, with the Final as the centre bullseye (shows the champion's flag + name once it's decided). Each resolved node writes one single radial label — "Team — Coach" if drafted in this pool, just the team name otherwise — into the open space toward the centre; still-undecided placeholder slots get a bare dashed node with no text at all, rather than repeating "Round of 32 8 Winner" across a whole ring. Every ring's title (Round of 32, Round of 16, ...) shows once in a reserved gap, not repeated on every node. Tap a node for a detail card below with score and live/FT status — nothing shows by default, so there's no arbitrary match called out before you've asked for one. Eliminated teams are dimmed and struck through in place; 3rd place is a small tappable note beneath the wheel since it isn't part of the winners' line. Ring/node order is reconstructed from the *real* bracket lineage (matching resolved teams back to their source fixture, and parsing ESPN's "Round of 32 8 Winner"-style placeholders for unresolved ones) — not just chronological kickoff order, which doesn't always match the actual pairing.
