...
- **Bracket** — a radial knockout tree (SVG): flag-badge nodes connected by standard bracket elbow lines (arc + radial line, like a normal left-to-right bracket bent into a circle), not a filled pie/donut chart. Rings run Round of 32 on the outside in to Semi-final, with the Final as the centre bullseye (shows the champion's flag + name once it's decided). Each node renders the team's real vector flag art (`flags.js`, not an emoji glyph — iOS Safari renders flag emoji as a blank box inside SVG `<text>`), ringed green once that team has won its tie or red once it's lost, with a smaller name (+ coach, if drafted in this pool) label just inside it — and the winner's branch of the connector into the next round is highlighted green so their path forward is obvious at a glance. Still-undecided placeholder slots get a bare dashed node with no text at all, rather than repeating "Round of 32 8 Winner" across a whole ring; every ring's title shows once in a reserved gap, not repeated on every node. Tap a node for a detail card below with score and live/FT status — nothing shows by default. 3rd place is a small tappable note beneath the wheel since it isn't part of the winners' line. Ring/node order is reconstructed from the *real* bracket lineage (matching resolved teams back to their source fixture, and parsing ESPN's placeholder names for unresolved ones) — not just chronological kickoff order, which doesn't always match the actual pairing.
