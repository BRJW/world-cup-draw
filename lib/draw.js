...
export function teamRecords(matches) {
  ...
  for (const m of matches) {
    ...
    // A knockout match level after 120 minutes is decided on penalties — that's
    // a win/loss for standings purposes, not a draw, even though the score line
    // stays level.
    if (m.scoreA === m.scoreB && m.shootout && (m.winnerA === true || m.winnerB === true)) {
      if (m.winnerA) { a.w++; a.pts += 3; b.l++; }
      else { b.w++; b.pts += 3; a.l++; }
    } else if (m.scoreA > m.scoreB) { a.w++; a.pts += 3; b.l++; }
...
// Knockout rounds, in order. A team's elimination is only knowable once it
// loses one of these outright (or on penalties) — group-stage exits aren't
// detected here, since that needs full 12-group standings with FIFA's
// tiebreaker chain (including fair-play points we have no data source for).
// A team is reported 'alive' right up until it actually loses a knockout tie.
export const KNOCKOUT_STAGES = ['Round of 32', 'Round of 16', 'Quarter-final', 'Semi-final', 'Third-place', 'Final'];

export function teamStatus(code, matches) {
  for (const m of matches) {
    if (!KNOCKOUT_STAGES.includes(m.stage)) continue;
    if (m.teamA !== code && m.teamB !== code) continue;
    if (!isFinal(m) || m.scoreA == null || m.scoreB == null) continue;
    const isA = m.teamA === code;
    const mine = isA ? m.scoreA : m.scoreB;
    const opp = isA ? m.scoreB : m.scoreA;
    if (mine < opp) return 'out';
    if (mine === opp && m.shootout && (m.winnerA === true || m.winnerB === true)) {
      const won = isA ? m.winnerA : m.winnerB;
      if (!won) return 'out';
    }
  }
  // Once the Round of 32 field is fully set — all 32 slots resolved to real
  // teams, not ESPN's "Group X Winner"-style placeholders — any of the 48
  // that ISN'T among them didn't survive the group stage. This reads off
  // ESPN's own standings/tiebreaker computation instead of reimplementing
  // FIFA's tiebreaker chain (goal difference, head-to-head, disciplinary
  // points, etc.) ourselves, which we have no reliable data source for.
  const r32 = matches.filter((m) => m.stage === 'Round of 32');
  if (r32.length) {
    const advanced = new Set();
    for (const m of r32) {
      if (teamByCode(m.teamA)) advanced.add(m.teamA);
      if (teamByCode(m.teamB)) advanced.add(m.teamB);
    }
    if (advanced.size >= 32 && !advanced.has(code)) return 'out';
  }
  return 'alive';
}
...
export function leaderboard(players, picks, matches) {
  ...
  for (const pick of picks) {
    ...
    row.teams.push({ ...team, record: r || null, status: teamStatus(pick.teamCode, matches) });
    if (r) {
