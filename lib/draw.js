...
//
// The draft runs in 4 rounds (snake order):
//   R1 "upper lead"  — random team from Tier 1 ∪ Tier 2
//   R2 "upper twin"  — its complement (Tier 1→4, Tier 2→3)
//   R3 "lower lead"  — random team from Tier 5 ∪ Tier 6
//   R4 "lower twin"  — its complement (Tier 5→8, Tier 6→7)
...
// Tier complement within each half.
const COMPLEMENT = { 1: 4, 2: 3, 5: 8, 6: 7 };

export const ROUND_INFO = [
  { round: 1, label: 'Upper headliner', blurb: 'A team from Tier 1 or 2' },
  { round: 2, label: 'Upper balancer', blurb: 'The complementary tier' },
  { round: 3, label: 'Lower pick', blurb: 'A team from Tier 5 or 6' },
  { round: 4, label: 'Lower balancer', blurb: 'The complementary tier' },
];
...
// Scoring / leaderboard
// ----------------------------------------------------------------------------
// A match counts toward standings only when it is FINAL: ESPN reports
// `completed` once full-time, and commissioner-entered (`manual`) results are
// treated as final too. Live (in-progress) scores are shown but never moved
// into the standings, so the leaderboard can't flip mid-match.
