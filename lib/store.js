...
class MemoryStore {
  ...
  async updatePlayer(playerId, fields) {
    ...
    if (!p) return null;
    for (const k of ['teamName', 'image', 'phone']) if (k in fields) p[k] = fields[k];
    return p;
  }
  ...
  async createLoginRequest({ phone, code, token, ttlMs }) {
    this.logins = this.logins.filter((r) => r.phone !== phone); // one active per phone
    const row = { token, phone, code, expiresAt: Date.now() + ttlMs, used: false };
    this.logins.push(row);
    return row;
  }

  async findLoginRequest({ token, phone, code }) {
    const now = Date.now();
    return this.logins.find((r) => !r.used && r.expiresAt > now &&
      ((token && r.token === token) || (phone && code && r.phone === phone && r.code === code))) || null;
  }
...
class PgStore {
  ...
  async updatePlayer(playerId, fields) {
    const map = { teamName: 'team_name', image: 'image', phone: 'phone' };
    const sets = [];
    const vals = [];
    let i = 1;
  ...
  async createLoginRequest({ phone, code, token, ttlMs }) {
    await this.pool.query('DELETE FROM login_requests WHERE phone=$1', [phone]);
    const expires = new Date(Date.now() + ttlMs).toISOString();
    await this.pool.query(
      'INSERT INTO login_requests (token, phone, code, expires_at) VALUES ($1,$2,$3,$4)',
      [token, phone, code, expires]
    );
    return { token, phone, code };
  }
  ...
  async findLoginRequest({ token, phone, code }) {
    ...
    if (token) {
      r = await this.pool.query(
        'SELECT * FROM login_requests WHERE token=$1 AND used=false AND expires_at>now()', [token]
      );
    } else {
      r = await this.pool.query(
        'SELECT * FROM login_requests WHERE phone=$1 AND code=$2 AND used=false AND expires_at>now()', [phone, code]
      );
    }
    const x = r.rows[0];
    return x ? { token: x.token, phone: x.phone, code: x.code } : null;
  }
