...
class MemoryStore {
  ...
  async sessionAddToken(id, token) {
    let s = this.sessions.get(id);
    if (!s) { s = { tokens: new Set(), updatedAt: Date.now() }; this.sessions.set(id, s); }
    s.tokens.add(token); s.updatedAt = Date.now();
  }

  async sessionTokens(id) {
...
class PgStore {
  ...
  async sessionAddToken(id, token) {
    ...
    const tokens = r.rows[0]?.tokens || [];
    if (!tokens.includes(token)) tokens.push(token);
    await this.pool.query(
      `INSERT INTO sessions (id, tokens, updated_at) VALUES ($1,$2,now())
       ON CONFLICT (id) DO UPDATE SET tokens=$2, updated_at=now()`,
